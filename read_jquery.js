...
( function( global, factory ) {
	...
	if ( typeof module === "object" && typeof module.exports === "object" ) {
		...
		module.exports = global.document ?
			...
			// 如果不是CommonJS环境，直接调用工厂方法创建jQuery对象
		factory( global );
	}

// 如果window还没有定义，则传入this
...
// 当非严格代码访问严格模式时，会抛出异常（如，ASP.NET 4.5），截止jQuery3.0，严格模式总是会被整个jQuery所使用
"use strict";
...
// 立即执行传入的js代码
	function DOMEval( code, doc ) {
		...
		// 首先将传入的代码赋值给新建的script标签
		script.text = code;
		// 在head标签中添加script标签，然后立即删除（标签添加就会将script内部的代码执行一遍）
...
var
	...
	jQuery = function( selector, context ) {
		// jQuery对象只是init构造函数的增强（或者简化版）
		// 如果调用了jQuery,则会使用init进行初始化
